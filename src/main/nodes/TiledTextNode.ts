...
export class TiledTextNode extends TextNode<Gather> {
    ...
    public constructor(args?: TiledSceneArgs) {
        const textProperty = args?.tiledObject?.getOptionalProperty("text", "string")?.getValue();
        const nameFallback = args?.tiledObject?.getName() ?? "";
        const text = (textProperty ?? nameFallback).toString();
        ...
        super({
            ...args,
            font: TiledTextNode.fonts[fontIndex],
            outlineColor,
            text,
            fallbackFont: "18px 'Segoe UI', 'Microsoft YaHei', 'PingFang SC', 'Noto Sans CJK SC', sans-serif",
